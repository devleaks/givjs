(function($) {
    ...
    var FlapDigit = function($ele, opts) {
        this.options = opts;
    ...
    const vestacolors = {
     ...
     const vestavalid = " ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$()^-_+&=;:*'" + '"' + ",.<>/" + String.fromCharCode(92) + "?|%"; // \ = ASCII 92
   const vestachars = vestavalid + Object.keys(vestacolors).join("");

    FlapDigit.prototype = {

        presets: {
            vestaboard: vestachars.split(""),
        ...
        goToChar: function(c) {
            ...
            if (pos == -1) {
                if (this.options.chars_preset == "vestaboard") { // in vestaboard, replace non existant chars with SPACE
